...
\begin{equation}
  ...
  &+ \beta_3 I(\text{treatment group}_{ij} = 4) + \beta_5 I(\text{treatment group}_{ij} = 5)\\
  &+ u_i + \epsilon_{ij}
  \end{split}
...
The $\hat{\beta_k}$, $k = 2,...,5$, will estimate the change in average value of pCREB immunoreactivity, with respect to the baseline, for the typical mouse.
...
Group 5 consists of the mice measured 1 week after being treated with ketamine. We estimate that mice measured 1 week after being treated have `r round(abs(mice_group5_est), 3)` lower pCREB immunoreactivity than the baseline group, on average, for the typical mouse.
...
+---------------------------------------------------+----------------------------------------------+--------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------+
| Continuous response variable with correlated data | Liner Mixed Effect Model (LME)               | $E[Y_{ij}] = \beta_0 + \beta_1 X_{ij} + u_i$           | Same as a in a linear model, but for the "typical" individual                                                                |
+---------------------------------------------------+----------------------------------------------+--------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------+
| Discrete response variable with correlated data   | Generalized Linear Mixed Effect Model (GLMM) | $g(E[Y_{ij}]) = \beta_0 + \beta_1 X_{ij} + u_i$        | Same as a in a generalized linear model, but for the "typical" individual                                                    |
+---------------------------------------------------+----------------------------------------------+--------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------+
